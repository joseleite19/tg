...
extern tnode *remove_from_parent(tnode *parent_parent, tnode *parent, tnode *position);
  ...
  if (s != NULL) {
	...
	if (s->next->formula->type == CONSTANT) {
	  ...
	  if (s->next != NULL &&
	    ...
	    s->formula->left->parent = s->parent;
	...
	if (s->next->formula->type == CONSTANT) {
	  ...
	  if (s->next != NULL &&
		...
		size = size_tree(s->formula->left->left);
	      ...
	      s->formula->left->left->parent = s->formula->left;
