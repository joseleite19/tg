...
extern prop_node *delete_hml_props(int type, int ml, prop_node *p, tnode *node);
extern int in (tnode *t1, formulalist *l);

tnode *free_tree(tnode *t);
formulalist *free_formulalist (formulalist *l);
...
int print_tree(tnode *s) {  
  ...
  if (s!=NULL) {
	...
	printf(",parent vn:%u)\n", s->parent->value_number);
      ...
      break;
    default:
      printf("We have a problem, Houston. Printing the tree. %p\n", s);
    }
  }
  return 0;
}
...
tnode *free_tnode(tnode *t) {
  if (t != NULL) {
	...
	if (positions != NULL) {
      ...
      prop_node *p = find_prop(t->id);
      p = delete_hml_props(t->polarity,t->mlevel,p,t);
    }
    else {
      if (t->left != NULL)
