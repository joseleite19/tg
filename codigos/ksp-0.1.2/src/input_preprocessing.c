...
tnode *remove_from_parent(tnode *parent_parent, tnode *parent, tnode *position) {
  if (parent_parent->left == parent)
    ...
    while (auxl != NULL && auxl->formula != parent)
      auxl = auxl->next;
...
int propagate_truth(tnode **node, tnode **parentnode) {
  ...
  if (position != NULL && parent != NULL) {
	...
	if (position == parent->left) { // true is on the left-hand side: true => phi = phi
	  parent->left = free_tree(parent->left);
	  parent->parent = remove_from_parent(parent->parent,parent,parent->right);
	  parent->right = NULL;
	  free(parent);
	  //	  parent = free_tree(parent);
	  numsimp++;
	  flag = 1;
	}
...
int do_pure_literal(void) {
  ...
  HASH_ITER(hid,propsid,p,tmpp) {
	...
	//	printf("\n Modal Level: %d",hmlpositions->ml);
	if ((early_ple == ON  && p->occur_positive != 0 && p->occur_negative == 0) ||
	    (early_mlple == ON && hmlpositions->ml_occur_positive != 0 &&  hmlpositions->ml_occur_negative == 0)) {
	  flag = 1;
	  //printf("\n POSITIVE: p, p->hml: %p,%p,%p",p,p->hmlpropsposition,hmlpositions);
	  ...
	  HASH_DELETE(hml,p->hmlpropsposition,hmlpositions);
	  hmlpositions->positive = NULL;
	  free(hmlpositions);
	}
	else if ((early_ple == ON  && p->occur_positive == 0 && p->occur_negative != 0)  ||
		 (early_mlple == ON && hmlpositions->ml_occur_positive == 0 && hmlpositions->ml_occur_negative != 0)) {
	  flag = 1;
	  //printf("\n NEGATIVE: p, p->hml: %p,%p,%p",p,p->hmlpropsposition,hmlpositions);
...
tnode *input_simplification (tnode *root) {
  ...
  if (bnfsimp == ON) {
    ...
    do {
      ...
      if (early_mlple == ON || early_ple == ON) 
	do_early_ple();
