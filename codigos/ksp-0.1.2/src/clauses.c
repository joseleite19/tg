...
void free_clauses_set_p(hml_pclauses **set) {
  hml_pclauses *aux = NULL, *auxtmp = NULL, *auxtmp2 = NULL;
  hmaxclauses *hmc = NULL, *hmctmp = NULL;
  hsizeclauses *hsc = NULL, *hsctmp = NULL;  
 
  aux = *set;

  HASH_ITER(hml,aux,auxtmp,auxtmp2) {
    HASH_ITER(hmax,auxtmp->max,hmc,hmctmp) {
      HASH_ITER(hsize,hmc->size,hsc,hsctmp) {
	hsc->list = delete_clauses_list(hsc->list);
	HASH_DELETE(hsize,hmc->size,hsc);
	free(hsc);
      }
      HASH_CLEAR(hsize,hmc->size);
      HASH_DELETE(hmax,auxtmp->max,hmc);
      free(hmc);
    }
    HASH_CLEAR(hmax,auxtmp->max);
    HASH_DELETE(hml,aux,auxtmp);
    free(auxtmp);
  }
  HASH_CLEAR(hml,aux);
  aux = NULL;
...
void free_clauses_set_m (hml_mclauses **set) {
  hml_mclauses *aux = NULL, *auxtmp = NULL, *auxtmp2 = NULL;
  ...
  HASH_ITER(hml,aux,auxtmp,auxtmp2) {
    HASH_ITER(hid,auxtmp->id,hag,hagtmp) {
	...
	HASH_CLEAR(hmax,htc->lit);
	HASH_DELETE(htype,hag->type,htc);
	free(htc);
      }
      HASH_CLEAR(htype,hag->type);
      HASH_DELETE(hid,auxtmp->id,hag);
      free(hag);
    }
    HASH_CLEAR(hid,auxtmp->id);
    HASH_DELETE(hml,aux,auxtmp);
    free(auxtmp);
  }
  HASH_CLEAR(hml,aux);
